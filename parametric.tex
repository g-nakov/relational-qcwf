...
% dirty fix, as section counter is bound to thm env 
...
Reflexive graphs provide enough structure for a dyadic interpretation of types in the spirit of \cite{Ma1992}.


\subsection{Reflexive graphs with realisability information}

One could easily generalize reflexive graphs by considering object and relation components from arbitrary categories instead of $\cSet$. As our purpose is to build a relational model incorporating realisability information, we replace the set of objects with an \assembly and retain the $\cSet$-based representation of relations.
...
Accordingly, \textbf{a morphism between \asmrgs, $G$ and $H$}, is a pair $(f\O: G\O \to H\O, f\R : G\R \to H\R)$, s.t. $(f\O, f\R)$ is an $\cRGph$ morphism between the reflexive graphs $(|G\O|, G\R, G\rfl, G\src, G\tgt)$ and $(|H\O|, H\R, H\rfl, H\src, H\tgt)$.\\
...
\begin{itemize}[noitemsep]
  \item $M\O \define \{f_\gamma : S\O(\gamma) \to T\O(\gamma) \}_{\gamma \in \Gamma\O}$
  %\footnote{\FNF{should be tracked?}}
  
  \item $M\R \define \{f_\gamma : S\R(\gamma) \to T\R(\gamma) \}_{\gamma \in \Gamma\R}$
\end{itemize}
s.t.\ the following identities are satisfied:
\begin{itemize}[noitemsep]
  \item $T\rfl(M\O(\gamma)(s_o)) = M\R(\Gamma\rfl(\gamma))(S\rfl(\gamma)(s_o))$ for every $\gamma \in \Gamma\O$, $s_o \in |S\O(\gamma)|$
  
  \item $T\src(M\R(\gamma)(s_r)) = M\O(\Gamma\src(\gamma)) (S\src(\gamma)(s_r))$ for every $\gamma \in \Gamma\R$, $s_r \in S\R(\gamma)$
  
  \item $T\tgt(M\R (\gamma)(s_r)) = M\O(\Gamma\tgt(\gamma))(S\tgt(\gamma)(s_r))$ for every $\gamma \in \Gamma\R$, $s_r \in S\R(\gamma)$
\end{itemize}
...
\begin{enumerate}[noitemsep]
  ...
  %\footnote{..., s.t. $U(emp_\pi)=id_1$ \FNF{since 1 terminal, this is automatic?}}
  
  \subitem $emp_+ : \lozenge \to \lozenge + \lozenge$
  %\footnote{..., s.t. $U(emp_+)=id_1$ \FNF{ditto}}
...
%\footnote{\FNF{would be good for clarity to spell this out (up to isomorphism, removing the unit type element)}}:
\begin{align*}
...
\begin{flalign*}
X \define\quad &\{ (f\O, f\R) \quad | \quad  f\O : \forall s \in |S\O(\gamma)|.T\O(\gamma, s),\quad \dots&& \\
...
\begin{flalign*}
&\mathbf{R} : \cORGph(\cAsm) \to \cORGph(\cAsm)&& \\
&\mathbf{R} (X\O, X\R, X\rfl, X\src, X\tgt) = ((\lfloor X\O \rfloor, \vDash_X), X\R, X\rfl, X\src, X\tgt)&&
\end{flalign*}
...
\begin{definition}[Comodest set/Distinct realisers?]
...
\begin{definition}[No universal realiser]
...
Now since $M \in RTm(\Gamma, T)$, $M$ is realizable - in particular, there exists an $a \in \algA$ that tracks $M(\gamma\O X(\gamma\O), -)$. By def.\ we get that $\forall x \in X\O, \forall b \in \algA. b \vDash_{\Gamma.0 X} (\gamma\O, x) \implies a . !_0 b \vDash_{X(\gamma\O)} M(\gamma\O, X(\gamma\O), x)$ - $a$ is a realizer for every element $x$ in $X\O(\gamma\O)$. But that is a contradiction, as no universal realizer exists for $X$ by assumption. Therefore no resourced term $M \overset{1}{:} T$ exists.\begin{definition}[Separability of realizers]
...
%$$
%\infer[\text{Conv}]{\Gamma \vdash \mathbf{elim}^{W\pi A B}_{C,c}(\mathbf{sup}_{A,B} a f) = c (\lambda b \overset{\pi} : Ba . \mathbf{elim}^{W\pi A B}_{C,c}(f b)) \overset{\sigma}: C[\mathbf{sup}_{A,B} a f / z]}{
%  \Gamma \vdash a \overset{\sigma} : A  
%  &
%  \Gamma \vdash f \overset{\sigma} : (\_ \overset{\pi} : B a ) \to  W \pi A B 
%}
%$$
