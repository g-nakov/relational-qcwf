...
\autodefs{\cCat\cSet\cRGph\cSet\bT\bU}
...
One could easily generalize reflexive graphs by considering object and relation components from arbitrary categories instead of $\cSet$. As our purpose is to build a realisability model, we replace the set of objects with an assembly$^\dagger$ $\cAsm(\algA)$ identify two appropriate notions of reflexive graph of assemblies and a family of reflexive graphs of assemblies.

\begin{definition}[Reflexive graph of assemblies]
  \FNF{It's not easy, but we should think of a good name for this --- ``reflexive graph of $X$'' usually means ``reflexive graph in the category of $X$s'', i.e.\ where $G\O$ and $G\R$ are $X$s, and the maps are $X$ morphisms.}
  A reflexive graph of assemblies $G$ is a tuple $(G\O, G\R, G\rfl, G\src, G\tgt)$, where $G\O$ is an assembly, $G\R$ - a set and the functions $G\rfl : \carr{G\O} \to G\R, \; G\src : \carr{G\R} \to G\O, \; G\tgt : \carr{G\R} \to G\O)$ are such that the identities in \cref{def:refl-graph} are satisfied.
\end{definition}
...
\begin{itemize}[noitemsep]
  \item $M\O \define \{f_\gamma : \carr{S\O(\gamma)} \to T\O(\gamma) \}_{\gamma \in \Gamma\O}$ \FNF{should be tracked?}
...
\begin{enumerate}[noitemsep]
  ...
  \subitem $emp_\pi : \lozenge \to \pi\lozenge$, s.t. $U(emp_\pi)=id_1$ \FNF{since 1 terminal, this is automatic?}
  \subitem $emp_+ : \lozenge \to \lozenge + \lozenge$, s.t. $U(emp_+)=id_1$ \FNF{ditto}
...
Take $\cL := \cRGph(\algA)$ \FNF{``half-assemblied'' r.g.s (yes, we also need notation for them)} and let $U$ be the functor $\cRGph(\algA) \to \cRGph$, sending an assembly$^\dagger$ to its underlying set, forgetting the realisability function.\\
...
\begin{itemize}
  ...
  \item $\carr{\Gamma'\R}$ is defined analogously. \FNF{no realizability anymore}
...
%    
%    %\textit{To wit that addition preserves realisability of functions, wlog take some $\gamma \in \Gamma\R$, s.t $ a \vDash_{\Gamma\R} \gamma$. Then by definition there exists $x, y\in \algA$, s.t $ x \vDash_{{\Gamma'}\R} \gamma$ and $y \vDash_{{\Gamma''}\R} \gamma$. Moreover, there exist
...
%    %To see that, for instance, $\Gamma\rfl$ retain realisability, let $a \vDash_{\Gamma\O} \gamma$. Therefore, there is some $x, b \in \algA$, s.t $ x \vDash_{\Gamma\O'} \gamma$ and $b. x \vDash_{\Gamma\R'} \Gamma'\rfl(\gamma)$. Now consider $a^* \define F_\rho. (!_\rho b)$, we have $a^* . x \vDash_{\Gamma\R} \Gamma\rfl(\gamma)$.
%    
%    As the functor $\rho$ modifies only realisability information, it is clear that $U(\rho(-)) = U(-)$ holds.
%    
%  \end{itemize}
%  
%\end{enumerate}
...
\begin{itemize}
  \item $\carr{U\O(\gamma)} \define \text{the set of small, discrete r.g. of assemblies}$ \\
   $a \vDash_{U\O(\gamma)} S$ for any $a \in \algA$, $S \in \carr{U\O(\gamma)}$. \FNF{Again maybe $a \vdash S$ iff $a = I$ is better for linearity reasons?}
