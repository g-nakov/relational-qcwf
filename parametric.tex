...
There is plethora of problems with the definition of $\Pi$ types. In order to satisfy the bijections posited in $\cref{def:prod-types}$, we had to include too many inhabitants of $(\Pi\pi S T)\O$. This leads to an internal conflict w.r.t realisability - some of the elements might not be realised at all, while others might share the same realiser. To see the latter, consider pairs $(f\O, f\R), (f'\O, f'\R) \in |(\Pi\pi S T)\O(\gamma)|$, such that $f\O$ and $f'\O$ have identical behaviour on $\lfloor S\O(\gamma) \rfloor$, but there exists some $s \in |S\O(\gamma)$, s.t $f\O(\gamma, s) \neq f'\O(\gamma,s)$.
