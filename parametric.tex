...
We present the explicit construction in our model. Fix some $\pi \in R$, let $\Gamma$ be a r.g in $\ob\cC$, $S \in Ty(\Gamma)$, $T \in Ty(\Gamma . S)$. 
Define the semantic type $\Pi \pi S T$ as the family of assemblies over $\Gamma$, consisting of:
\begin{itemize}
    \item $(\Pi \pi S T)\O(\gamma) \define (X, \vDash_X)$ for $\gamma \in \Gamma\O$.
    ...
    \subitem
     $\begin{aligned}
           Y \define \{( (f\O^{src}, \, f\R^{src}), (f\O^{tgt}, f\O^{tgt}), r)  \quad|\quad
...
Given a reflexive graph $\Gamma$, $S \in Ty(\Gamma)$, $T \in Ty(\Gamma.S)$, $\rho \in R$, define $\Pi\rho S T \in Ty(\Gamma)$ as the tuple of:
%
%\begin{itemize}[noitemsep]
