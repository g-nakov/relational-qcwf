...
One could easily generalize reflexive graphs by considering object and relation components from an arbitrary category $\cC$ instead of $\cSet$. As our purpose is to build a realizability model, we pick $\cAsm(\algA)$ as the category of interest and identify two appropriate notions of a family of reflexive graphs of assemblies and internal r.g. in $\cAsm(\algA)$.

\begin{definition}[Internal reflexive graph in $\cAsm(\algA)$]
  An internal reflexive graph $G$ in $\cAsm(\algA)$ is a pair of assemblies $(G\O, G\R)$ and a triple of morphisms $(G\rfl : G\O \to G\R, \; G\src : G\R \to G\O, \; G\tgt : G\R \to G\O)$, such that the identities in \cref{def:refl-graph} are satisfied.
\end{definition}
